...
# df[['variation_MFOP ((m3/s)/(m3/s))','variation_TVA (m3)','STATUS']][1:].plot()
...
# data[['variation_MFOP ((m3/s)/(m3/s))','variation_TVA (m3)','STATUS']][1:].plot()
...
def feature_rep(window):
    ...
    elif window['variation_TVA (m3)'].sum() > 0.110:
        tva = True
...
kick_res=np.ones(df.shape[0])

result = []
for i, batch in windows:
   
    # batch.loc[:, 'kick_recognition'] = kick_detection(feature_rep(batch))
    # df.loc[batch.index, 'kick_recognition'] = batch['kick_recognition']
    # result.append(kick_detection(feature_rep(batch)))
    window_varation(batch,wind_var)

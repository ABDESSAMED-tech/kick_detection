...
var={'variation_TVA':[], 'variation_SPPA':[], 'variation_MFOP':[],'GASA (mol/mol)':[]}
def feature_rep(window, thrsh_tva, thrsh_gasa, thrsh_spp, thrsh_mfop, p):
    ...
    if p:
        var['variation_TVA'].append(tva)
        var['GASA (mol/mol)'].append(gasa)
        var['variation_SPPA'].append(spp)
        var['variation_MFOP'].append(mfop)
        
        
# gasa pas de variasion
    detect_gasa, detect_spp, detect_tva, detect_mfop = False, False, False, False
    l = []
    if  gasa > thrsh_gasa and tva > thrsh_tva and  spp < thrsh_spp:  # well 29
    ...
    # if gasa > thrsh_gasa and tva > thrsh_tva and mfop < thrsh_mfop:  # well 17,19
    #     l = [int(gasa > thrsh_gasa), int(tva > thrsh_tva),
    #           int(mfop < thrsh_mfop), int(spp < thrsh_spp)]
...
thrsh_tva, thrsh_gasa, thrsh_spp, thrsh_mfop =0.08,0.04, 1,0.001#accuracy:  0.9259172154280338 accuracy status 0.3121951219512195
...
thrsh_tva, thrsh_gasa, thrsh_spp, thrsh_mfop =0.08,0.04, 1,0.00#29
# thrsh_tva, thrsh_gasa, thrsh_spp, thrsh_mfop = 0.1,0.1,0, 0.005#29
window_size = 1000
window_size = window_size//5
positive=0
train_data=[]
index=data[data['STATUS']==1].index
d=[]
